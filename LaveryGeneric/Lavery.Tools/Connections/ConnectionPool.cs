...
namespace Lavery.Tools.ConnectionPool
{
    ...
    public class ConnectionPool<T,Y>    where T: IGenericConnection 
                                        where Y: IGenericConnection, new()
    {
        ...
        const int ConstMaxWaitIteration = 100;
        ...
        public Boolean releaseGenericConnection(T oWithConnection)
        {
            ...
            try
            {
                if (oWithConnection != null)
                {
                    ...
                    int iCountLoop = ConstMaxWaitIteration;
                    while (!bRet )
                    {
                        lock (syncLock)
                        {
                            if (lLogicalConnection.Count == 0 || iCountLoop-- < 0)
                            {
                                throw (new Exception("this connection is not managed by this Pool..."));
                            }
                            foreach (ConnexionBase<T> oConnectionType in lLogicalConnection)
                            {
