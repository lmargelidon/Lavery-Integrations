...
namespace Lavery.Tools.DataBaseClassLibrary
{
    public class CTableForSqlServer : CTable
    {
        ...
        public CTableForSqlServer(String sWithSchema, String sWithName, String sWithAlias, CDBConnectionFactory pWithConnFactory)
            : base(pWithConnFactory, sWithSchema, sWithName)
        {   
            pRS = new CResultSetForSqlServer(pWithConnFactory);
        ...
        {
            pRS = new CResultSetForSqlServer(pWithDB.PConnFactory, pWithDB.SConnectString);
            sSqlCommand = "select * from " + (sWithSchema.Length > 0 ? sWithSchema + "." : "") + sWithName;
        ...
        public override bool Reset(String sWithSqlCommand)
        {
            ...
            try
            {
                ...
                cmdSql.Connection = ((CDBConnectionForSqlServer)pDb.PConnFactory.getConnection(ConnType.eConnType.SQLSERVER, pDb.SConnectString, 1000)).Conn;
        ...
        public override bool Drop()
        {
            ...
            try
            {
                CDBConnectionForSqlServer pConnection = (CDBConnectionForSqlServer)pConnFactory.getConnection(ConnType.eConnType.SQLSERVER, 1000);
                SqlCommand mySqlCommand = new SqlCommand();
                mySqlCommand.Connection = pConnection.Conn;
        ...
        {   object oRet = null;
            try
            {
                CDBConnectionForSqlServer pConnection = null;
                if (pDb != null)
                    pConnection = (CDBConnectionForSqlServer)pConnFactory.getConnection(ConnType.eConnType.SQLSERVER, pDb.SConnectString, 1000);
                else
                    pConnection = (CDBConnectionForSqlServer)pConnFactory.getConnection(ConnType.eConnType.SQLSERVER, 1000);
